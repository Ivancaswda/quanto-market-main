            ...
            <div>
                {
                    orders.reverse().map((order, index) => (
                        <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_2fr_1fr_0.5fr] gap-3 items-start border-2
                        border-blue-100 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700' key={index}>
                                    ...
                                    <div>
                                        {order.items.map((item, index) => {
                                            if (index === order.items.length - 1) { // getting last product in order
                                                return (
                                                    <p className='py-0.5' key={index}>{item.name} x {item.quantity}
                                                         <span></span></p>
                                                        )
                                            } else {
                                                return (
                            ...
                            <div>
                                ...
                                <p>Дата : <span
                                    className='text-blue-700'>{new Date(order.date).toLocaleDateString()}</span> </p> {/* displaying data */}
                                </div>
