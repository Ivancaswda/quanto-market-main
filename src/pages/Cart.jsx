...
const Cart = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        flat: '',
        phone: '',
        wayOfDelivery: 'в quantShop'
    })
    const onClickHandler = (event) => {

        const { value } = event.target;  // Получаем новое значение
        setFormData(prevData => ({
            ...prevData,
            wayOfDelivery: value  // Обновляем только поле wayOfDelivery
        }));
        console.log(formData.wayOfDelivery)
    }
    ...
    const onSubmitHandler = async (event) => {
        event.preventDefault();
            ...
            if (formData.email === '' || formData.name === '' || formData.surname === '' || formData.phone === '') {
                    toast.warn('Укажите необходимые данные!')
                return;
            }

        // Проверка: если способ доставки "курьером", проверяем, что все обязательные поля заполнены
        if (formData.wayOfDelivery === 'Курьером' || formData.wayOfDelivery === 'Quant-express') {
            if (!formData.street || !formData.home || !formData.flat) {
                toast.warn(`Пожалуйста, укажите улицу, дом и квартиру для доставки ${formData.wayOfDelivery}`);
                return;
            }
        }

        // Проверка на выбран ли способ доставки
        if (formData.wayOfDelivery === '') {
            toast.warn('Выберите способ получения товара');
            return;
        }
        ...
        try {
            setLoading(true);
            switch (method) {
                case 'cod':  // обработка метода "наложенный платеж"
            ...
            { !loading ? (
                <form onSubmit={onSubmitHandler} className='border-t pt-14 '>
                    <div className='text-2xl mb-3'>
                        <h1 className='flex items-center gap-2 p-4 font-semibold'>Ваша корзина <svg width='20px'
                                                                                                    height='20px'
                    ...
                    {cartData.length > 0 ? (
                        <div>
                            <div className='w-100 block md:flex  items-start'>
                                                ...
                                                <div
                                                    ...
                                                    <div className=' block md:flex items-start gap-6 w-full'>
                                                        <LazyLoad>
                                                            ...
                                                            <img src={productData.image[0]} className='w-24  sm:w-22'
                                                                 alt=""/>

                                                        </LazyLoad>
                                                        <div className='w-100'>
                                                            <div className=' block md:flex justify-between items-center '>
                                                                <p className='text-xs sm:text-lg text-md font-medium mb-2 md:mb-0'>{productData.name}</p>
                                                                ...
                                                                <div className='flex items-center'>
                                                                    <div
                                                                        className='p-2  text-blue-700 font-semibold text-lg rounded-lg'>
                                                            ...
                                                            <div
                                                                className='flex flex-col items-start justify-start gap-5 mt-2 '>


                                                                <p title='Нажмите чтобы увеличить/уменьшить кол-во символов'
                                                                ...
                                                                <div
                                                                    ...
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16"
                                                                         height="16"
                                                                         fill="currentColor"
                                                                         className="bi bi-bookmark-heart-fill cursor-pointer"
                                                                         viewBox="0 0 16 16">
                                                                        <path
                                ...
                                <div className='flex  justify-left md:justify-end bg-gray-100 p-6 rounded-2xl '>
                                    <div className='w-full sm:w-[300px]'>
                                        <CartTotal/>
                                        <div className='w-full text-left md:text-center flex items-start md:items-center justify-left md:justify-center mt-4'>


                                            <button
                                                className='bg-blue-700 text-white text-lg px-8 md:px-16 py-2 rounded-lg'>Перейти
                                                к
                                                оплате
                                            </button>
                            ...
                            <div className=' p-4 w-full md:w-78  ' >


                                {token ?
                                    (<div className='bg-gray-100 p-4 w-full md:w-[85%]  ' >

                                        <div className='flex mb-4 '>
                                            <h1 className='text-2xl text-left font-semibold '>Способ Доставки</h1>
                                        </div>
                                        <div id='deli' className='block md:flex items-center gap-2  background'>
                                            <div className='block  lg:flex items-center gap-2'>
                                                <button
                                                    className='py-2 mb-2 px-4 bg-blue-700 rounded-lg text-sm text-white flex gap-2 items-center font-semibold'
                                                    type='button' name='wayOfDelivery' onClick={onClickHandler}
                                                    value={'В quantShop'}>
                                        ...
                                        <div className='flex items-center  mb-8'>
                                            <div className='flex items-center gap-2'>
                                                <h1 className='text-lg font-semibold'>Что бы выбрать способ доставки у
                                                    вас
                                                    должен быть <span className='text-blue-700'>аккаунт</span></h1>
                                                <button onClick={() => {
                                                    navigate('/login')
                                                }}
                          ...
                          {cleanedEmail ? cleanedEmail : 'Введите ваш email!'}
                                                    ...
                                                    <input required={true} name='email' value={formData.email}
                          ...
                          {'Введите ваше имя!'}
                                                    ...
                                                    <input required={true} name='name' value={formData.name}
                                                           onChange={onChangeHandler}
                                                           className='border border-zinc-300 rounded w-full p-1 text-sm mt-1 text-gray-700'
                                                           type='text'/>
                                                )}
                          ...
                          {'Введите вашу фамилию!'}
                                        ...
                                        {formData.wayOfDelivery !== 'В quantShop' && (
                                            <>
                        ...
                        <div style={{height: '50vh'}} className='w-100 flex items-start justify-center text-center '>
                            <div className='flex flex-col gap-6 items-center'>
                                ...
                                <div className={'flex flex-col gap-2'}>
                                    ...
                                    <p className='text-1xl'>Загляните на главную, чтобы выбрать товары или найдите
                                        нужное в
                                        поиске</p>
                                </div>
                                <Link to='/'>
                                    <button className='  px-4 py-2 bg-blue-700 text-white rounded-lg'>Перейти на главную
                                    </button>
                                </Link>
                            </div>
                        </div>
