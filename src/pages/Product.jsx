...
const Product = () => {
    ...
    const {products, currency, addToCart,  proceedBtn, setProceedBtn } = useContext(ShopContext)
    ...
    useEffect(() => {
        if (products.length > 0) {
            ...
            productsCopy = productsCopy.filter((item) => { // get filtered product by subCategory
                return productData._id !== item._id
            })
  ...
  //  const {handleReviewAdded, reviews} = useContext(ShopContext)
    ...
    const getReviewData =  async () => {
        ...
        try {
            ...
            const response =  await axios.get(backendUrl + `/api/reviews/get?productId=${productId}`)
            ...
            // Получаем выровненный массив
            const flattenedArray = flattenDeep(reviews);

            // Подсчитываем количество элементов
            const count = flattenedArray.length;


            localStorage.setItem(`countOfReview-${productId}`, count)
        } catch (error) {
    ...
    return productData ? (
        <div className='border-t-2 pt-20 pb-20 transition-opacity ease-in duration-500 opacity-100 '>
            ...
            <div className='mt-20 overflow-x-auto w-100'>
                <div className='flex justify-between items-center'>
                    <b className='border px-5 py-3 text-sm'>Описание</b>
                    <b className='border px-5 py-3 text-sm '>Оставить отзыв о продукте</b>

                </div>
                <div className='lg:flex md:flex  justify-between sm:block block w-100 gap-4 border px-6 py-5 text-sm text-gray-500'>

                    <div className='w-50 overflow-hidden'>
                        <p className='w-100'>{productData.description}...</p>

                    </div>
                    <div className='w-50 sm:mt-4'>
                        <AddReview productId={productData._id} reviews={reviews} getReviewData={getReviewData} />

                    </div>

                </div>
                <Reviews productId={productData._id} getReviewData={getReviewData} reviews={reviews}/>
            </div>
            {/*----------related Products----------- */}
            ...
            <div className='my-24'>
                ...
                { relatedProducts.length !== 0 ? (
                    ...
                    Похожих продуктов не существует!
                </h1>}
            </div>
