...
const NewsletterBox = () => {
    ...
    return (
        <div className='text-center  h-100 flex items-center justify-center flex-col pl-4 pr-4 md:pl-20 md:pr-20'>
            ...
            <svg className='w-[60px] h-[60px] md:w-[140px] md:h-[140px]' xmlns="http://www.w3.org/2000/svg"
                 viewBox="0 0 576 512">
                <path fill="#0634a3"
            ...
            <p className='text-sm md:text-2xl font-medium text-gray-800'>
                Создай аккаунт сейчас & получи -20% скидку
            </p>
            <p className='text-sm md:text-xl text-gray-400 mt-3'>
                Схвати возможность получить скидку 20% на покупки в нашем онлайн магазине до {today + 3} {monthName} {year } года!
            </p>
            <form onSubmit={(e) => {
                e.preventDefault()
            }} className='w-100 sm:w-1/2 block md:flex items-center   my-6 '>
                <input required={true} className='w-100 sm:flex-1 text-sm outline-none px-2 py-3 md:px-3 md:py-4'
                       placeholder='Введите промокод для продолжения' type="password"/>
                <button type='submit' style={{background: 'rgb(3, 50, 160)'}}
                        className=' text-white text-sm md:text-xs px-2 py-3  sm:mt-677 mt-1 sm:px-2 sm:py-3 md:px-1 md:py-4'>Отправится к регистрации
                </button>
            </form>
        </div>
    )
